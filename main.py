...
def require_env(name: str) -> str:
    ...
    if not value:
        raise PushError(f"缺少 GitHub Secret / 环境变量：{name}")
    return value
...
def load_config() -> Dict[str, Any]:
    ...
    if not users:
        raise PushError("WECHAT_USER_OPENIDS 为空。它应该是测试号关注者的 OpenID，多个用英文逗号分隔。")
...
def request_json(url: str, params: Dict[str, Any] = None, method: str = "GET", body: Dict[str, Any] = None) -> Dict[str, Any]:
    try:
        if method == "POST":
...
def get_access_token(config: Dict[str, Any]) -> str:
    ...
    print("access_token 接口返回：", mask_sensitive(data))

    if "access_token" not in data:
        raise PushError(f"获取 access_token 失败：{data}")
...
def get_weather_3d(config: Dict[str, Any], location_id: str) -> List[Dict[str, Any]]:
    ...
    data = request_json(url, params=params)
    print("3天天气预报接口返回：", data)

    if data.get("code") != "200" or not data.get("daily"):
        raise PushError(f"3天天气预报获取失败：{data}")

    return data["daily"]


def get_air_now(config: Dict[str, Any], location_id: str) -> Dict[str, Any]:
...
def has_any_weather(text: str, words: List[str]) -> bool:
    return any(word in text for word in words)


def make_clothing_advice(now: Dict[str, Any], today: Dict[str, Any], tomorrow: Dict[str, Any], air: Dict[str, Any]) -> str:
    now_temp = to_int(now.get("temp"))
    now_humidity = to_int(now.get("humidity"))
    min_temp = to_int(tomorrow.get("tempMin"), now_temp)
    max_temp = to_int(tomorrow.get("tempMax"), now_temp)
    humidity = to_int(tomorrow.get("humidity"), now_humidity)
    ...
    if min_temp <= 0:
        advices.append("明天非常冷，建议羽绒服/厚棉服、毛衣或保暖内衣都安排上，围巾、手套、厚袜子也别忘。")
    elif min_temp <= 6:
    ...
    if temp_diff >= 10:
        advices.append(f"昼夜温差约{temp_diff}℃，建议采用洋葱式穿搭：里面轻薄，外面加外套，冷了能穿、热了能脱。")

    if has_any_weather(weather_text, ["雷", "雷阵雨"]):
        advices.append("有雷电相关天气，打雷害怕的话尽量早点回家，关好窗，别在树下、空旷处或水边停留；晚上可以开小夜灯，放点轻音乐，安心睡。")

    if has_any_weather(weather_text, ["暴雨", "大雨", "中雨", "阵雨", "小雨", "雨"]):
        advices.append("可能下雨，记得带伞；鞋子尽量穿防滑、不怕湿的，裤脚别太长，回家后及时擦干头发和脚。")

    if precip >= 5:
        advices.append(f"预报降水量约{precip}mm，雨感会比较明显，包里放纸巾，通勤路上慢一点。")

    if has_any_weather(weather_text, ["雪", "雨夹雪"]):
    ...
    if min_temp <= 3:
        advices.append("低温天早起别猛地出门，先喝点温水，外套拉链扣好，别让冷风直灌。")

    if humidity >= 85:
        advices.append(f"湿度约{humidity}%，会有点闷湿，衣服不容易干，出门可以带纸巾，回家注意通风。")
    elif humidity <= 35:
        advices.append(f"湿度约{humidity}%，空气偏干，记得多喝水，护唇膏和护手霜可以安排。")

    if any(ch in wind_scale for ch in ["4", "5", "6", "7", "8", "9"]):
        advices.append(f"明天{wind_dir}{wind_scale}级，风感会明显，裙摆和头发都要照顾一下，外套最好选防风的。")
    ...
    aqi = to_int(air.get("aqi"), 0)
    category = air.get("category", "")

    if aqi >= 150:
        advices.append(f"空气质量{category or aqi}，不建议长时间户外活动，敏感的话戴口罩。")
    elif aqi >= 100:
        advices.append(f"空气质量一般，敏感体质可以戴口罩，少在路边久待。")

    if min_temp <= 12 or temp_diff >= 8 or has_any_weather(weather_text, ["雨", "雪", "降温", "冷"]):
        advices.append("感冒提醒：早晚别贪凉，淋雨后要及时换干衣服；嗓子不舒服就少喝冰的，多喝温水，早点休息。")
...
def build_message(config: Dict[str, Any]) -> Dict[str, str]:
    ...
    air = get_air_now(config, location_id)
    ...
    temperature_humidity = (
        ...
        f"体感{now.get('feelsLike')}℃，湿度{now.get('humidity')}%，"
        f"{now.get('windDir')} {now.get('windScale')}级。"
    )

    if air:
        temperature_humidity += f" 空气质量：{air.get('category', '未知')}，AQI {air.get('aqi', '未知')}。"
    ...
    return {
        ...
        "clothing_advice": make_clothing_advice(now, today, tomorrow, air),
...
def send_template_message(config: Dict[str, Any], access_token: str, to_user: str, message: Dict[str, str]) -> None:
    ...
    data = request_json(url, params=params, method="POST", body=payload)
    print(f"发送给 {mask_openid(to_user)} 的微信接口返回：", data)

    if data.get("errcode") != 0:
        raise PushError(f"模板消息发送失败：{data}")
...
def mask_sensitive(data: Dict[str, Any]) -> Dict[str, Any]:
    cloned = dict(data)
    if "access_token" in cloned:
        cloned["access_token"] = cloned["access_token"][:8] + "***"
    return cloned
